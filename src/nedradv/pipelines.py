...
class NedradvPipeline:
    async def process_item(self, item: scrapy.Item, spider: scrapy.Spider):
        async with AsyncSessionLocal() as session:
            ...
            db_auction = await auction_crud.get(session, site_id=parsed_item['auction_site_id'])
            db_holder = await holder_crud.get(session, name=parsed_item['auction_holder'])
            if not db_holder:
                db_holder = await holder_crud.create(
                    session, HolderCreate(name=parsed_item['auction_holder'])
                )
            if parsed_item['auction_region']:
                reg = parsed_item['auction_region']
                reg
                region_db = await region_crud.get(session=session, name=parsed_item['auction_region'])
                if not region_db:
