...
@pytest.mark.parametrize(
    ...
    argvalues=[
        ...
        ('№ 1740 от 20.12.21', date(2021, 12, 20)),
    ],
)
...
@pytest.mark.parametrize(
    ...
    argvalues=[
        (
            date(2023, 10, 2),
            AuctionStatus.OPEN,
            'Электронная площадка «ЭТП ГПБ» (www.etpgpb.ru), оператором которой является '
            'ООО «Электронная торговая площадка ГПБ». Дата — 2 октября 2023, в 11:00'
        ...
        (
            ...
            AuctionStatus.OPEN,
            'Электронная площадка «ЭТП ГПБ» (www.etpgpb.ru), оператором которой является '
            'ООО «Электронная торговая площадка ГПБ». Дата — 19 июня 2023 года, в 00:09'
            ' (время московское).',
        ),
    ],
)
...
@pytest.mark.parametrize(
    ...
    argvalues=[
        ...
        (
            1676594739.06,
            '1 676 594 739,06 (один миллиард шестьсот семьдесят шесть миллионов пятьсот'
            ' девяносто четыре тысячи семьсот тридцать девять рублей и 06 копеек)',
        ),
        (1.25, '1,25'),
    ],
)
